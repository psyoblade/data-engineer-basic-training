...
* 임의의 파일을 추가하고 스테이징 상태까지 갔다가 다시 원래 상태로 되돌립니다
```bash
bash> touch XXX
...
* 임의의 파일을 추가한 뒤, 커밋 이후에 다시 원래 상태로 롤백합니다
```bash
bash> touch YYY
bash> git status -sb
...
* 임의의 가비지 파일을 10개 생성하고, 깃 클린 명령어를 통해 정리합니다
```bash
bash> mkdir -p tmp
